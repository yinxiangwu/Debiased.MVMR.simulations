# Simulations for adIVW, three exposures example with horizontal pleiotropy
...
for (m in 1:nrow(sim_settings)) {
      res.beta1 <- matrix(nrow = n_rep, ncol = 19)
      res.beta2 <- matrix(nrow = n_rep, ncol = 19)
      res.beta3 <- matrix(nrow = n_rep, ncol = 19)
      set.seed(213 + task_id + m*10000)
      # setting
      D = sim_settings[m,]$D
      rho = sim_settings[m,]$rho
      beta = beta_l[[sim_settings[m,]$beta]]
      par.beta.exposure[,1]<-rawdat_mvmr$LDL_beta/D
      par.beta.exposure[,2]<-rawdat_mvmr$HDL_beta/D
      par.beta.exposure[,3]<-rawdat_mvmr$Trg_beta/D
      ...
      tau0 <- sum(se.outcome)/p*2
      alpha0 <- rnorm(p, mean = 0, sd = tau0)
      par.beta.outcome <- par.beta.exposure %*% beta + alpha0
      beta.outcome<-rnorm(p,mean=par.beta.outcome,sd=se.outcome + tau0)
      ...
      # dIVW
      res.mvmr.divw <- mvmr.divw(beta.exposure, se.exposure, beta.outcome, se.outcome, gen_cor = P,over.dispersion = TRUE)
      res.dmvmr<- res.mvmr.divw$beta.hat
      res.dmvmr.se <- res.mvmr.divw$beta.se
      tau2_divw <- res.mvmr.divw$tau.square
      # adIVW
      res.mvmr.adivw <- mvmr.divw(beta.exposure, se.exposure, beta.outcome, se.outcome, gen_cor = P,phi_cand = NULL,over.dispersion = TRUE)
      ...
      tau2_adivw <- res.mvmr.adivw$tau.square
      # IVW
      ...
      write.csv(res.beta1, file = paste0("Res/final/3exp_example/all_divided_by_D_bhp/beta",sim_settings[m,]$beta,'/rho',sim_settings[m,]$rho,'/res_beta1_D',sim_settings[m,]$D,'_job',task_id,'.csv'), row.names = FALSE)
      write.csv(res.beta2, file = paste0("Res/final/3exp_example/all_divided_by_D_bhp/beta",sim_settings[m,]$beta,'/rho',sim_settings[m,]$rho,'/res_beta2_D',sim_settings[m,]$D,'_job',task_id,'.csv'), row.names = FALSE)
      write.csv(res.beta3, file = paste0("Res/final/3exp_example/all_divided_by_D_bhp/beta",sim_settings[m,]$beta,'/rho',sim_settings[m,]$rho,'/res_beta3_D',sim_settings[m,]$D,'_job',task_id,'.csv'), row.names = FALSE)
